...
import ConfirmModals from '@/components/modals/confirm-modals';
...
const TrashBox = () => {
    ...
    const filtredDocuments = documents?.filter((document)=> document.title.toLocaleLowerCase().includes(search.toLowerCase()))
                ...
                filtredDocuments?.map((document)=>(
                    <div key={document._id} role='button' onClick={()=>onClick(document._id)} className='text-sm rounded-sm w-full hover:bg-primary/5 flex items-center text-primary justify-between'>
                        ...
                        <div className='flex items-center'>
                            ...
                            <ConfirmModals onConfirm={()=>onRemove(document._id)}>
